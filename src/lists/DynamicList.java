...
public class DynamicList {
	...
	/**
	 * protected void decreaseCapacity() { int newCapacity = elements.length /
	 * 2; if (newCapacity < 10) { if (elements.length <= 10) return; newCapacity
	 * = 10; } Element[] temp = new Element[newCapacity]; for (int index = 0;
	 * index < newCapacity; index++) { temp[index] = elements[index]; } elements
	 * = temp; }
	 */
	...
	/**
	 * Deletes element with specified key from the array if such an element
	 * exists. Elements are shifted to ensure there is no empty space in the
	 * list. When the array is less than 1/3 full the capacity of the underlying
	 * structure is shrunk to half the size. (Minimum size will be 10.)
	 * 
	 * @param key
	...
	public Element delete(int key) {
		...
		/**
		 * if (size < elements.length/3) { decreaseCapacity(); }
		 */
